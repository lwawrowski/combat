...
test_that("check results", {
  expect_equal(length(hexs_mobility(hex_id = "ID9", units_data = units_game, dist_matrix_data = dist_matrix)), 127)
  expect_equal(hexs_mobility(hex_id = "ID352", units_data = units_game, dist_matrix_data = dist_matrix)[1], c("ID210" = 8))
  expect_equal(length(hexs_mobility(hex_id = "ID352",mobility = 2, units_data = units_game, dist_matrix_data = dist_matrix)), 9)
