...
```{r}
hexs_range(hex_id = "ID8", range_min = 1, range_max = 10, dist_matrix_data = dist_matrix)
```

It is also possible to check what would be an effect of attack based on attack points of unit:

```{r}
hp_check(player_id = "ID8", opponent_id = "ID2", units_data = units_game, dist_matrix_data = dist_matrix)
```
